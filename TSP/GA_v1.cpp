...
const double A = -6881114;            // 适应度函数尺度变换参数
const double K = 40.0;                // 适应度函数尺度变换参数
const double earth_radius = 6375.000; // 地球半径
const int city_num = 34;              // 城市总数目
const int population_size = 130;      // 种群规模
const int num_of_iteration = 1200;     // 迭代次数
const int group_size = 4;             // 锦标赛每组随机选择个体数
const double cross_coef1 = 0.45;      // 自适应交叉概率系数（针对较好个体）
...
public:
    ...
    Status Cross_Over(Chrome &another_chrome)
    {
        ...
        std::uniform_int_distribution<unsigned> u2(city_num/3, (2* city_num)/3);
        ...
        if (greater_fitness > avg_fit) // 这意味着两个个体质量较高，应当降低交叉概率
        {
            cross_odds = (cross_coef1 * (best_fit - greater_fitness)) / (best_fit - avg_fit);
        }
        else // 这意味着两个个体质量较差，应当提高交叉概率
        {
            cross_odds = cross_coef2;
        }

        // 当交叉概率过低时，设置为下限值，避免局部最优
        if (cross_odds < cross_floor)
        {
            cross_odds = cross_floor;
        }
        ...
        {
            // 得到随机断点，断点后的片段将参与交叉
            int break_point = u2(e);

            // 存储两个剪切下来的片段，以建立映射
            vector<City> cut_part1;
            vector<City> cut_part2;

            // 保存切片，并逐一交换基因
            for (int j = break_point; j < city_num; j++)
            {
                // 依次序保存交换部分，准备 PMX
                cut_part1.push_back(city_array$[j]);
                cut_part2.push_back(another_chrome.city_array$[j]);
            ...
            {
                vector<City>::iterator cpt = find(cut_part2.begin(), cut_part2.end(), city_array$[j]);
                while (cpt != cut_part2.end()) // 仍存在冲突
                {
                    city_array$[j] = cut_part1[cpt - cut_part2.begin()];            // 映射赋值
                    cpt = find(cut_part2.begin(), cut_part2.end(), city_array$[j]); // 继续针对该点寻找
                }
            }
            for (int j = 0; j < break_point; j++)
            {
                vector<City>::iterator cpt = find(cut_part1.begin(), cut_part1.end(), another_chrome.city_array$[j]);
                while (cpt != cut_part1.end()) // 仍存在冲突
                {
                    another_chrome.city_array$[j] = cut_part2[cpt - cut_part1.begin()];            // 映射赋值
                    cpt = find(cut_part1.begin(), cut_part1.end(), another_chrome.city_array$[j]); // 继续针对该点寻找
                }
            }
    ...
    Status Mutation()
    {
        ...
        if (fitness$ > avg_fit) // 这意味着两个个体质量较高，应当降低变异概率
        {
            mutation_odds = (mutation_coef1 * (best_fit - fitness$)) / (best_fit - avg_fit);
        }
        else // 这意味着两个个体质量较差，应当提高变异概率
        {
            mutation_odds = mutation_coef2;
        }

        // 当变异概率过低时，设定为下限值，避免局部最优
        if (mutation_odds < mutation_floor)
        {
            mutation_odds = mutation_floor;
        }
        ...
        if (mutation_odds >= mutate_g)
        {
            ...
            while (mutate_point1 == mutate_point2) // 不相同
            {
                mutate_point2 = u1(e);
            }

            // 互换位置
            City tt(city_array$[mutate_point1]);
            city_array$[mutate_point1] = city_array$[mutate_point2];
            city_array$[mutate_point2] = tt;
...
int main()
{
    ...
    for (int i = 0; i < num_of_iteration; i++)
    {
        ...
        for (int j = 0; j < population_size / 2; j++)
        {
            int cross_id_1 = u(e);
            int cross_id_2 = u(e);
            while (cross_id_1 == cross_id_2 || cross_id_1 == best_id || cross_id_2 == best_id) // 最佳个体不参与交叉
            {
                cross_id_1 = u(e);
                cross_id_2 = u(e);
            }
        ...
        {
            int mutate_id = u(e);
            while (mutate_id == best_id) // 最佳个体不参与交叉
            {
                mutate_id = u(e);
            }
