...
const double cross_probability = 0.25;    // 交叉概率
...
int main()
{
    ...
    for (int i = 0; i < num_of_iteration; i++)
    {
        ...
        // 寻找适应度最大的个体
        int best_id = DEFAULT_SET, best_fit = DEFAULT_SET;
        for(int i = 0; i< population_size; i++)
        {
            if(origin[i].Get_Fitness() > best_fit)
            {
                best_id = i;
                best_fit = origin[i].Get_Fitness();
            }
        }
    ...
    // 输出各代中最优个体
    sort(bests_history.begin(), bests_history.end(), Fitness_Greater);
    printf("Best point is: \n");
    printf("(%.5lf, %.5lf);", bests_history[0].Get_X1(), bests_history[0].Get_X2());
    printf("\nFitness: %.5lf.\n", bests_history[0].Get_Fitness());
