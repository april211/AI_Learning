...
const int mach_num = 4;                  // 机器数目
const int jobs_num = 5;                  // 工件数目
const double cross_probability = 0.6;    // 交叉概率
const double mutation_probability = 0.1; // 变异概率
const int population_size = 40;          // 种群规模
const int num_of_iteration = 50;         // 迭代次数
const int cross_group_num = 4;           // 参与交叉的组数
const int best_time = 213;               // 最优解时的时间
typedef int Status;
...
Status initialize(vector<vector<Job>> &orders, vector<Job> &single, vector<Job> &job_finder)
{
    ...
    e1.seed((unsigned int)time(NULL) * random_disturb);
...
Status jobs_proc_time(vector<vector<Job>> &orders, int size)
{
...
Status select(vector<vector<Job>> &orders, vector<Info> &chrome_list1, vector<vector<Job>> &random_chosen) // 个体适应度、种群
{
    ...
    e2.seed((unsigned int)time(NULL) * random_disturb);
    ...
    // 更新种群中个体的完成时间
    jobs_proc_time(random_chosen, population_size);
...
Status cross_over(vector<vector<Job>> &random_chosen, vector<Info> &chrome_list1)
{
    ...
    e.seed((unsigned int)time(NULL) * random_disturb);
...
int main()
{
    ...
    for (int k = 0; k < 100; k++)
    {
        ...
        // 最优历史记录
        vector<vector<Job>> best_history;
        ...
        {
            // 求种群中个体的完成时间
            jobs_proc_time(orders, population_size);
            ...
            // 记录本次迭代的最优个体
            best_history.push_back(random_chosen[chrome_list1[0].id]);
        ...
        // 求种群中个体的完成时间
        jobs_proc_time(best_history, num_of_iteration);

        chrome_fitnesses(best_history, chrome_list2, num_of_iteration);
        ...
        {
            printf("%d ", best_history[best_id][j].id$); //***
        }
        printf("\nFitness: %lf, procedure time: %d.\n", chrome_list2[0].fitness, chrome_list2[0].maxc);
        printf("Iteration num: %d\n", best_solution_itnum);

        if (chrome_list2[0].maxc == best_time)
        {
            ++best_cnt;
        }
    }
    printf("\n--------------\n");
    printf("Succeeded ratio: %.2lf.\n", (best_cnt * 1.0) / 100.0);
