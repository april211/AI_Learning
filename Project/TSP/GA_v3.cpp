...
const double A = -6381114;            // 适应度函数尺度变换参数
...
const double cross_coef2 = 0.5;       // 自适应交叉概率系数
const double cross_floor = 0.6;       // 自适应交叉概率门槛值
const double mutation_coef1 = 0.4;    // 自适应变异概率系数
const double mutation_coef2 = 0.2;    // 自适应变异概率系数
const double mutation_floor = 0.4;    // 自适应变异概率门槛值
const int threshold_min = 440;        // 迭代门槛数下限
const int threshold_max = 500;        // 迭代门槛数上限
typedef int Status;
...
public:
    ...
    Status Cross_Over(Chrome &another_chrome)
    {
        ...
        // 避免局部最优
        if (threshold_max > iter && iter > threshold_min)
        {
        ...
        if (cross_odds >= cross_g)
        {
            ...
            // 交换部分的长度
            int cross_len = city_num - break_point;

            // 冲突消解，使用 PMX消解方法（共两组）
            // 寻找是否有重复基因（冲突点）
            for (int j = 0; j < break_point; j++)
            {
                vector<City>::iterator cpt = find(city_array$.begin() + break_point, city_array$.end(), city_array$[j]);
                while (cpt != city_array$.end()) // 仍存在冲突
                {
                    city_array$[j] = another_chrome.city_array$[cpt - city_array$.begin()];           // 映射赋值
                    cpt = find(city_array$.begin() + break_point, city_array$.end(), city_array$[j]); // 继续针对该点寻找
                }
            }
            for (int j = 0; j < break_point; j++)
            {
                vector<City>::iterator cpt = find(another_chrome.city_array$.begin() + break_point, another_chrome.city_array$.end(), another_chrome.city_array$[j]);
                while (cpt != another_chrome.city_array$.end()) // 仍存在冲突
                {
                    another_chrome.city_array$[j] = city_array$[cpt - another_chrome.city_array$.begin()];                                         // 映射赋值
                    cpt = find(another_chrome.city_array$.begin() + break_point, another_chrome.city_array$.end(), another_chrome.city_array$[j]); // 继续针对该点寻找
                }
            }
